...
- `apps/web/lib/slack/repo-parser.ts`, `apps/web/lib/slack/link-token.ts`, `apps/web/lib/slack/session-kickoff.ts`, `apps/web/lib/slack/replyback.ts` - add focused Slack helpers for parsing `repo=owner/repo#branch <prompt>` (with missing `#branch` meaning new-branch mode), signed link tokens, session creation/orchestration, and final-thread posting.
...
- Manual end-to-end:
  - @mention the bot with `repo=owner/repo#branch <prompt>` (or omit `#branch` to create a new branch)
